...
import { db, categories, ticket_statuses, domains } from '@/db';
...
export async function GET() {
    try {
        await requireRole(['super_admin', 'snr_admin', 'admin', 'committee']);
        const [cats, stats, activeDomains] = await Promise.all([
