...
function SnrAdminTicketSkeleton() {
  return (
...
// Note: User sync is already handled in snr-admin/layout.tsx, so user should exist here
async function SnrAdminTicketAuthed({ params }: { params: Promise<{ ticketId: string }> }) {
  const { userId } = await auth();
  if (!userId) redirect("/snr-admin/dashboard");
  ...
  try {
    ...
    const ticketData = await getCachedAdminTicketData(id, 'snr-admin', userId);
    ...
    console.error('[SnrAdminTicketAuthed] Error:', error);
 ...
 */
export default function SnrAdminTicketPage({ params }: { params: Promise<{ ticketId: string }> }) {
  // Render shell immediately - no auth, no DB, no blocking
  return (
    <Suspense fallback={<SnrAdminTicketSkeleton />}>
      <SnrAdminTicketAuthed params={params} />
    </Suspense>
  );
}
