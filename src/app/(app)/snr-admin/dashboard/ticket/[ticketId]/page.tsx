...
export default async function SnrAdminTicketPage({ params }: { params: Promise<{ ticketId: string }> }) {
    ...
    // For Senior Admin, allow forwarding to Super Admins and other Senior Admins/Admins
    ...
    const forwardTargets = forwardTargetsRaw
        ...
        .map((admin) => ({
            id: admin.id!,
            name: `${admin.full_name || admin.email || "Admin"} (${admin.role_name})`,
            email: admin.email,
        }));
    ...
    const genericEscalationIndex = timelineEntries.findIndex((e: any) => e.title && e.title.startsWith('Escalated to Level'));
                                                ...
                                                <div className="flex-1 min-w-0 pb-4">
                                                    <div className="p-3 rounded-lg bg-muted/50 border">
                                                        ...
                                                        {(() => {
                                                            ...
                                                            return description && (
                                                                <p className="text-xs text-muted-foreground mb-2 break-words">{description}</p>
                                                            );
                                                        })()}
                                                        {entryDate && (
