...
/**
 * Senior Admin Today Pending Page
 * Note: Auth and role checks are handled by snr-admin/layout.tsx
 */
export default async function SnrAdminTodayPendingPage() {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {todayPending.map((t) => (
              <div key={t.id} className={overdueTodayIds.has(t.id) ? "ring-2 ring-orange-400 dark:ring-orange-500 rounded-lg" : ""}>
                ...
                } as unknown as Ticket & { status?: string | null; category_name?: string | null; creator_name?: string | null; creator_email?: string | null }} basePath="/snr-admin/dashboard" />
              </div>
            ))}
          </div>
        </div>
