...
export function FieldOptionsEditor({
  ...
  const handleOptionChange = (index: number, key: "label" | "value", value: string) => {
    ...
    if (key === "label") {
      const currentValue = newOptions[index]?.value || "";
      const previousLabelSlug = current.label ? generateSlug(current.label) : "";
      
      // Auto-generate if:
      // 1. Value is empty, OR
      // 2. Value matches the slug of the previous label (was auto-generated), OR
      // 3. Value is a single character (likely from partial typing)
      const wasAutoGenerated = !currentValue || currentValue === previousLabelSlug || currentValue.length === 1;
      
      if (wasAutoGenerated) {
        if (value.trim().length > 0) {
          // Generate slug from the new label
          newOptions[index]!.value = generateSlug(value);
        } else {
          // Clear value if label is cleared
