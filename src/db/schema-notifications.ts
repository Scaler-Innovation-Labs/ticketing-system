...
// ============================================
// Category Assignments (Admin-to-Category mapping)
// ============================================
...
export const category_assignments = pgTable('category_assignments', {
  ...
  // Assignment type: 'primary', 'backup', 'escalation'
  assignment_type: varchar('assignment_type', { length: 32 }).default('primary'),
  
  created_at: timestamp('created_at').notNull().defaultNow(),
}, (table) => ({
  categoryIdx: index('category_assignments_category_idx').on(table.category_id),
  userIdx: index('category_assignments_user_idx').on(table.user_id),
  uniqueAssignment: uniqueIndex('category_assignments_unique_idx').on(table.category_id, table.user_id),
}));
